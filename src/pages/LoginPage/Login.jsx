...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call handleLogin from context to update Header state
      handleLogin({
    ...
    <div className="min-h-screen bg-gray-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-md bg-white p-8 rounded-xl shadow-lg space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-6" aria-label="Login form">
          ...
          <button
            ...
            className={`w-full py-2 px-4 rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base ${
              isSubmitting || loginAttempts >= 3 ? 'opacity-50 cursor-not-allowed' : ''
            }`}
        ...
        <p className="text-center text-sm text-gray-600">
          No account?{' '}
          <Link to="/signup" className="text-blue-600 hover:text-blue-500" aria-label="Sign up">
            Sign up
          </Link>
        </p>
      </div>
